...
int ledfd[3];
unsigned long long int if_info[8]; // current interface values.
unsigned long long int l_if_info[8]; // last interface values.
	...
	while (fgets(b,sizeof(b),procfd) !=  NULL) {
		...
		if (strncmp(bp,interface,strlen(interface)) == 0 && *(bp+strlen(interface)) == ':' ) {
			...
			if (options & OPT_KERNEL_2_0)
				sscanf(bp,"%llu %llu %llu %c %c %llu %llu %llu %c %llu",
				...
				sscanf(bp,"%llu %c %llu %llu %c %c %c %c %llu %c %llu %llu %c %llu",
...
int main(int argc, char *argv[]) {
	...
	int led;
	for (led=0;led < 3;led++) {
		sprintf(fd, "/sys/class/leds/apu2:green:led%d/brightness", led+1);
